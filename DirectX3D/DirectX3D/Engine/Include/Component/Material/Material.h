...
typedef struct GP_DLL _tagTextureInfo
{
...
public:
	void SetMaterial(const Vector4& vDif, const Vector4& vAmb, const Vector4& vSpc, const Vector4& vEmv, float fPower);
	void SetMaterial(int iShaderConstantType);

	void SetDiffuseTexInfo(const string& strSmpKey, const string& strDifKey,
		int iRegTex = 0, int iRegSmp = 0, const TCHAR* pFileName = NULL,
		const string& strPathKey = TEXTURE_PATH);
	void SetDiffuseTexInfo(const string& strSmpKey, const string& strDifKey,
		int iRegTex = 0, int iRegSmp = 0, const vector<wstring>* vecFileName = NULL,
		const string& strPathKey = TEXTURE_PATH);
