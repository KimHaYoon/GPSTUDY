...
// 직교 투영 
void CCamera::SetOrthoProj(const RESOLUTION & tRS, float fNear, float fFar)
{
	/*
	2/(r-l)      0            0           0
	0            2/(t-b)      0           0
	...
	*m_matProj = XMMatrixOrthographicOffCenterLH(0.f, tRS.iWidth, tRS.iHeight, 0.f, fNear, fFar);
...
{
	*m_matProj = XMMatrixPerspectiveFovLH(fViewAngle, fAspect, fNear, fFar);
	...
	if (m_pAttach)
	{
		...
		if (m_eCameraType == CT_ORTHO)
		{
			if (m_tWorldRS.iWidth != UINT_MAX && m_tWorldRS.iHeight != UINT_MAX)
			{
				...
				// 좌우로 움직일 수 있는 영역 안에 타겟이 있을 경우 움직일 수 있게 한다.
		...
		else
		{
			...
			m_pTransform->SetWorldPos(vMove);
		}
	}
