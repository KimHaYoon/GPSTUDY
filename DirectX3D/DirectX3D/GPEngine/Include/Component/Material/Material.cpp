...
CMaterial::CMaterial()
{
	...
	m_tMtrl.vDif = Vector4::One;
	m_tMtrl.vAmb = Vector4::One;
	m_tMtrl.vSpc = Vector4(1.f, 1.f, 1.f, 3.2f);
	m_tMtrl.vEmv = Vector4::Zero;
}
...
void CMaterial::SetMaterial(const Vector4 & vDif, const Vector4 & vAmb, const Vector4 & vSpc, const Vector4 & vEmv, float fPower)
{
	m_tMtrl.vDif = vDif;
	m_tMtrl.vAmb = vAmb;
	m_tMtrl.vSpc = vSpc;
...
{
	GET_SINGLE(CShaderManager)->UpdateCBuffer(MATERIAL_BUFFER, &m_tMtrl, iShaderConstantType);
...
void CMaterial::SetDiffuseTexInfo(const string & strSmpKey, const string & strDifKey, int iRegTex, int iRegSmp, const TCHAR * pFileName, const string & strPathKey)
{
	SAFE_RELEASE(m_tDifInfo.pTexture);
	SAFE_RELEASE(m_tDifInfo.pSampler);
...
void CMaterial::SetDiffuseTexInfo(const string & strSmpKey, const string & strDifKey, int iRegTex, int iRegSmp, const vector<wstring>* vecFileName, const string & strPathKey)
{
	SAFE_RELEASE(m_tDifInfo.pTexture);
	SAFE_RELEASE(m_tDifInfo.pSampler);
